...
const defaultConfig: EmbedConfig = { selected_models: [], time_window: '24h', refresh_interval: 60, theme: 'system' }
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [statusResponse, probeResponse] = await Promise.all([
        fetch(`${apiUrl}/api/model-status/embed/status/batch?window=${nextConfig.time_window}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(nextConfig.selected_models || []) }),
      ...
      setProbeSummary(probeData.data || { enabled: false, running: false, results: [] })
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="mx-auto max-w-6xl px-4 py-5 sm:px-6 sm:py-8">
      ...
      <section className="surface overflow-hidden">
        <div className="surface-header"><div><h2 className="surface-title">模型状态</h2><p className="mt-0.5 text-xs text-muted-foreground">灰色表示无请求，状态色由原始计数在浏览器中计算</p></div><div className="flex items-center gap-1.5 text-xs text-muted-foreground"><Clock3 className="h-3.5 w-3.5" />{updatedAt ? formatRelativeTime(updatedAt) : '正在读取'}</div></div>
        {loading && !statuses.length ? <div className="flex h-64 items-center justify-center text-sm text-muted-foreground"><RefreshCw className="mr-2 h-4 w-4 animate-spin" />正在读取状态</div> : statuses.length ? <div className="divide-y divide-border">{statuses.map((status) => {
          ...
          return <article key={status.model} className="p-4 sm:p-5">
            ...
            <div className="mt-2 flex justify-between text-[11px] text-muted-foreground"><span>{config.time_window} 前</span>{probe && <span>主动探测 {probe.chat_ok ? `${probe.latency_ms} ms` : probe.error_code || '异常'}</span>}<span>现在</span></div>
          </article>
